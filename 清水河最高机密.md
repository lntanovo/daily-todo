...
- 暖色纸张风格的中英混排界面。
- 精简首页日期栏，显示公历、农历和星期；通过年月日快速日历查看其他日期。
- 单日任务和连续日期范围任务。
- 日期范围任务每天分别记录完成状态。
- 任务优先级：紧急为红色，普通为蓝色。
- 可选每日开始时间和结束时间。
...
- 首页品牌右侧的本地番茄钟，支持暂停、继续、提前结束、休息、任务关联和最近七天统计。
- 30 多句内置短句分类、直接选择和最多 80 字的用户自定义短句。
- 固定在左下角的设置入口，以及从右侧展开的设置抽屉。
...
背景是一个固定定位的 `<img>`，配合 CSS 的 `cover` 方式铺满窗口。全页暖色遮罩为 40%，标题、日期导航和任务清单直接显示在背景图上，不使用局部白色衬底；使用图片背景时也取消任务行的交替底色。页头品牌只显示正体大写 `TO DO LIST`，品牌右侧放置番茄钟主控，右侧保留“回到今天”和“新建事项”。图片背景上的正文和辅助文字使用更深的颜色，并通过八向、零模糊的 1px 浅色文字阴影形成细描边，隔开图片线条；小标签提高到 13px、600 字重。实色主要按钮和选中日期不加描边。思源宋体与霞鹜文楷保持不变，实际可读性仍会随用户图片的明暗和纹理变化。
...
现有版本标签记录了阶段性发布。本文档对应的当前产品版本为 `v1.3.3`。
...
| 番茄钟逻辑与界面 | `src/focus-core.js`、`src/focus.js`、`src/focus.css` |
| 日期显示与农历 | `src/date-display.js` |
| 语录库与切换 | `src/quote-catalog.js`、`src/quotes.js` |
| 设置抽屉 | `src/settings.js`、`src/settings.css` |
