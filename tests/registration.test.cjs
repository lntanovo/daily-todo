...
const input = { nickname: '测试朋友', relation: '自动化测试', password: 'TestOnly9!safe', requestToken: 'a'.repeat(64), website: '' };
const entry = { code: 'RESERVED', username: 'ln_' + 'b'.repeat(16), uid: 'lnreg_' + 'b'.repeat(48) };
function mocks(code = 'RESERVED') {
  const calls = [];
  return { calls, bindingKey: 'local-test-binding-key',
    rpc: async (name, args) => { calls.push({ name, args }); return name === 'reserve_friend_registration' ? { ...entry, code } : true; },
...
test('validation accepts intended fields and rejects malformed inputs', () => {
  ...
  for (const bad of [null, [], {}, { ...input, nickname: 'x' }, { ...input, relation: ' ' }, { ...input, requestToken: 'guessable' }, { ...input, website: 'spam' }, { ...input, password: 'alllowercase' }, { ...input, password: '!TestOnly9' }, { ...input, password: 'Aaa123\n' }, { ...input, nickname: 'bad\nname' }]) assert.equal(validate(bad), null);
});
...
for (const code of ['CLOSED', 'RATE_LIMIT', 'DAILY_LIMIT', 'TOTAL_LIMIT', 'BUSY', 'CHANGED']) {
...
test('duplicate after uncertain timeout recovers exact identity, never resets password', async () => {
  ...
  assert.equal((await register(input, deps)).username, entry.username);
  deps.findUser = async () => 'some-other-user';
  await assert.rejects(register(input, deps), /IDENTITY_MISMATCH/);
});
